...
namespace Nop.Plugin.Widgets.Employees
{
    public class EmployeesPlugin : BasePlugin, IWidgetPlugin
    {
        ...
        internal void CreateLocaleStrings()
        {
            ...
            {
                this.AddOrUpdatePluginLocaleResource(res.Item1, res.Item2);

                if (islId >= 0)
                {
                    var lsr = localizationService.GetLocaleStringResourceByName(res.Item1, islId, false);
                    lsr.ResourceValue = res.Item3;
